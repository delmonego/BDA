SET DATEFORMAT YMD --setando o padrão de data: ano, mes e dia
GO
USE master 
go
...
1 a cada inserção automática no INSERT, com CONSTRAINT de chave primária para
criar indice unico*/
GrupoNome VARCHAR(50),--VARCHAR = o tamanho é o dos dados inseridos
GrupoRazaoSocial VARCHAR(50),
GrupoCNPJ VARCHAR(20),
)
/*Usa-se PRIMARY KEY para identificar a linha de uma tabela
A variavel que carrega a PRIMARY é o identificador principal do indice da linha*/

INSERT Grupo --Inserir dados; não esquecer do VALUES após inserir as colunas
			 --no INSERT
        (
		  --não inserir valor no IDENTITY (GrupoCodigo)	
          GrupoNome,
          GrupoRazaoSocial ,
          GrupoCNPJ
		)  
